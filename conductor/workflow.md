...
1.  **RED**: Write a failing test in the `tests/` directory (using `pytest`).
2.  **GREEN**: Write the minimal code needed in `src/` to make the test pass.
3.  **REFACTOR**: Clean up the code while ensuring all tests stay green.

### 2. Tools & Standards

- **Framework**: `pytest-django`.
- **Data Generation**: `factory_boy` (avoid fixtures).
- **ORM Optimization**: Mandatory use of `select_related()` and `prefetch_related()` as per `django-expert` skill to prevent N+1 queries.
- **Security**: Strict adherence to `django-security` mandates:
  - `DEBUG = False` in production/staging.
  - Mandatory CSRF, XSS, and HSTS headers.
  - Password validators with minimum 12 characters.
  - Use of `Argon2` for password hashing.
- **Coverage Goal**: >80% for models and views.
...
## Project Context & Execution Environment

### 1. Operating System & Containerization

- **Host OS**: Windows (Win32).
- **Runtime Environment**: Docker (Linux containers via Docker Desktop).
- **Execution Protocol**: All commands related to the application (migrations, tests, shell, management commands) MUST be executed through the Docker Compose bridge using `docker-compose exec web <command>`. Never run Django commands directly on the host machine.
- **Path Handling**: Use Windows paths for host operations and forward slashes for container paths.
